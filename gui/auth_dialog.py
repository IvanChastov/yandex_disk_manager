...
from core.yandex.auth import (
    get_auth_url, get_token_by_code_detailed, test_token
)
...
"""
        ...
        self.code_entry = tk.Text(
            code_frame, height=3, width=50, wrap=tk.WORD
        )
        self.code_entry.pack(fill=tk.X, pady=5)

        # Добавляем скроллбар для текстового поля
        scrollbar = ttk.Scrollbar(
            code_frame, orient=tk.VERTICAL, command=self.code_entry.yview
        )
    ...
    def get_token(self):
        ...
        if isinstance(result, dict) and result.get('success', False):
            ...
            if self.token:
                ...
                if test_token(self.token):
                    ...
                    if current_user:
                        if save_token_to_user(current_user, self.token):
                            ...
                            self.status_var.set(
                                "Токен получен, но не сохранён"
                            )
                    else:
                        self.status_var.set(
                            "Пользователь не найден, токен не сохранён"
                        )
                ...
                self.status_var.set(
                    "Не удалось получить токен (пустой ответ)"
                )
                self.status_label.config(foreground="red")
        else:
            ...
            messagebox.showerror(
                "Ошибка", f"Не удалось получить токен:\n{error_msg}"
            )
    ...
    def on_closing(self):
        ...
        if hasattr(self, 'token') and self.token:
            ...
            if messagebox.askyesno(
                "Подтверждение", "Вы не получили токен. Выйти?"
            ):
                self.dialog.destroy()
