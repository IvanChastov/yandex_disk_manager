...
class TagAssignDialog:
    ...
    def create_widgets(self):
        ...
        ttk.Label(
            main,
            text="Выберите теги для файла:",
            font=('Arial', 10, 'bold')
        ).pack(anchor=tk.W)

        file_name = self.file_path.split('/')[-1]
        ttk.Label(
            main, text=file_name, foreground="blue"
        ).pack(anchor=tk.W, pady=5)

        ttk.Button(
            main, text="+ Создать новый тег", command=self.add_tag
        ...
        self.listbox = tk.Listbox(
            frame, selectmode=tk.MULTIPLE, height=12, font=('Arial', 10)
        )
        ...
        ttk.Button(
            btn_frame, text="Сохранить", command=self.on_ok, width=12
        ).pack(side=tk.RIGHT, padx=5)
        ttk.Button(
            btn_frame, text="Отмена", command=self.on_cancel, width=12
        ).pack(side=tk.RIGHT)
    ...
    def add_tag(self):
        ...
        name = simpledialog.askstring(
            "Новый тег", "Введите название тега:", parent=self.dialog
        )
        if name:
