...
class PreviewDialog:
    ...
    TEXT_EXTENSIONS = {
        '.txt', '.py', '.json', '.xml', '.html', '.css', '.js',
        '.md', '.log', '.docx'
    }
    CAD_EXTENSIONS = {'.dwg', '.dxf'}
    PDF_EXTENSIONS = {'.pdf'}

    def __init__(self, parent, file_item, client=None):
        self.parent = parent
        ...
        # Если клиент не передан, пытаемся создать через токен из файла
        if not self.client:
            # Пытаемся загрузить корпоративный токен
                ...
                if token:
                    try:
                        ...
                        messagebox.showerror(
                            "Ошибка", f"Не удалось создать клиент: {e}"
                        )
                        return
            else:
                messagebox.showerror("Ошибка", "Не найден файл с токеном")
                return
    ...
    def create_widgets(self):
        ...
        ttk.Label(
            info_frame, text="Файл:", font=('Arial', 9, 'bold')
        ).pack(side=tk.LEFT)
        ttk.Label(info_frame, text=self.file_name).pack(side=tk.LEFT, padx=5)

        ttk.Label(
            info_frame, text="Тип:", font=('Arial', 9, 'bold')
        ).pack(side=tk.LEFT, padx=(20, 0))
        ttk.Label(
            info_frame,
            text=self.file_ext.upper() if self.file_ext else "Неизвестно"
    ...
    def load_preview(self):
        ...
        if not self.client:
            self.preview_label.config(
                text="Ошибка: не удалось создать клиент"
            )
            return
        ...
        try:
            ...
            self.temp_file = os.path.join(
                temp_dir,
                f"preview_{os.urandom(8).hex()}_{self.file_name}"
            )
            ...
            if self.file_ext in self.IMAGE_EXTENSIONS:
                ...
                self.preview_label.config(
                    text=(
                        f"Предпросмотр для файлов типа {self.file_ext} "
                        "не поддерживается.\n"
                        "Нажмите 'Открыть в программе' для просмотра."
                    )
                )
    ...
    def show_image_preview(self):
        ...
        try:
            ...
            size_info = (
                ...
                f"(отображено: {new_width}x{new_height})"
            )
            ttk.Label(
                self.preview_frame, text=size_info, foreground="gray"
            ).pack(side=tk.BOTTOM)

        except Exception as e:
            self.preview_label.config(
                text=f"Ошибка отображения изображения: {e}"
            )
    ...
    def show_text_preview(self):
        ...
        try:
            ...
            # Читаем файл
            with open(self.temp_file, 'r', encoding='utf-8',
                      errors='replace') as f:
            ...
            if len(content) >= 10000:
                info = ttk.Label(
                    self.preview_frame,
                    text="(Показано первые 10000 символов)",
                    foreground="gray"
                )
            ...
            self.preview_label.config(
                text=f"Ошибка чтения текстового файла: {e}"
            )

    def show_cad_preview(self):
        """Отображает информацию о CAD-файле"""
        # Очищаем фрейм
        ...
        ttk.Label(
            info_frame,
            text=(
                "Для просмотра этого файла требуется специализированное ПО.\n"
                "Нажмите кнопку ниже, чтобы открыть файл "
    ...
    def show_pdf_preview(self):
        """Отображает информацию о PDF-файле"""
        # Очищаем фрейм
        for widget in self.preview_frame.winfo_children():
            widget.destroy()
    ...
    def open_cad(self):
        ...
        if found_program:
            try:
                ...
                messagebox.showerror(
                    "Ошибка", f"Не удалось открыть файл в CAD:\n{e}"
                )
        else:
