...
class AdminDialog:
    ...
    def create_widgets(self):
        ...
        ttk.Button(
            btn_frame, text="Добавить", command=self.add_user
        ).pack(side=tk.LEFT, padx=5)
        ttk.Button(
            btn_frame, text="Редактировать", command=self.edit_user
        ).pack(side=tk.LEFT, padx=5)
        ttk.Button(
            btn_frame, text="Удалить", command=self.delete_user
        ).pack(side=tk.LEFT, padx=5)
        ...
        # Кнопка закрытия
        ttk.Button(
            main, text="Закрыть", command=self.dialog.destroy
            ).pack(pady=10)
    ...
    def delete_user(self):
        ...
        if messagebox.askyesno(
            "Подтверждение", f"Удалить пользователя '{user.username}'?"
        ):
            user.delete()
            self.load_users()
...
class UserEditDialog:
    ...
    def create_widgets(self):
        ...
        ttk.Checkbutton(
            main, text="Активен", variable=self.active_var
        ...
        ttk.Button(
            btn_frame, text="Отмена", command=self.cancel
        ).pack(side=tk.RIGHT, padx=5)

    def load_user_data(self):
        """Загружает данные пользователя в поля"""
        if self.user:
