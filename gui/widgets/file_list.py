...
class FileListWidget(ttk.Frame):
    ...
    def create_widgets(self):
        ...
        title = ttk.Label(
            self, text="Файлы и папки", font=('Arial', 10, 'bold')
        )
        title.pack(pady=5)
    ...
    def update_tags_from_db(self):
        ...
        for item_id, data in self._items_data.items():
            try:
                ...
                tags_str = ", ".join(
                    [tag.name for tag in file_obj.tags.all()]
                )
    ...
    def on_context_menu(self, event):
        ...
        menu.add_command(
            label="Скачать", command=self.on_download_selected
        )
        menu.add_command(
            label="Предпросмотр", command=self.on_preview_selected
        )
        menu.add_separator()

        if self.can_manage_tags:
            menu.add_command(
                label="Назначить теги", command=self.on_assign_tags
            )

        if self.can_delete:
            menu.add_separator()
            menu.add_command(
                label="Удалить", command=self.on_delete_selected
            )
    ...
    def on_assign_tags(self):
        """Назначение тегов выбранному файлу"""
        selected = self.get_selected_items()
