...
class SettingsDialog:
    ...
    def create_widgets(self):
        ...
        ttk.Label(
            monitor_frame, text="Интервал проверки (секунд):"
        ).pack(anchor=tk.W)
        ...
        ttk.Label(
            interval_frame, text="секунд (мин: 30, макс: 3600)"
        ).pack(side=tk.LEFT, padx=5)

        # --- Настройки уведомлений ---
        notifications_frame = ttk.LabelFrame(
            main, text="Уведомления", padding=10
        )
        ...
        user_text = f"Текущий пользователь: {current_user or 'не авторизован'}"
        ttk.Label(user_frame, text=user_text).pack(anchor=tk.W)
        ...
        # Информация о ролях
        roles_frame = ttk.LabelFrame(
            main, text="Роли пользователей", padding=10
        )
        roles_frame.pack(fill=tk.X, pady=5)

        roles_text = (
            "Admin: полный доступ "
            "(просмотр, загрузка, удаление, управление тегами)\n"
            "Manager: просмотр, загрузка, управление тегами\n"
            "Viewer: только просмотр"
        )
        ttk.Label(
            roles_frame, text=roles_text, justify=tk.LEFT).pack(anchor=tk.W)
    ...
    def run(self):
        """Запускает диалог и возвращает результат"""
